...
export async function getWeightSeries(patientId: string): Promise<FitPoint[]> {
  const [{ data: meas }, { data: checks }] = await Promise.all([
    supabase.from("fit_measurements").select("value, measured_at").eq("patient_id", patientId).eq("metric", "weight_kg"),
  ...
  for (const m of (meas as { value: number; measured_at: string }[] | null) ?? []) points.push({ date: m.measured_at, value: Number(m.value) });
...
export async function getMeasurementSeries(patientId: string, metric: string): Promise<FitPoint[]> {
  ...
  return ((data as { value: number; measured_at: string }[] | null) ?? []).map((r) => ({ date: r.measured_at, value: Number(r.value) }));
}

export async function listPatientMetrics(patientId: string): Promise<FitMetricRef[]> {
  const { data } = await supabase.from("fit_measurements").select("metric, label, unit, category").eq("patient_id", patientId);
  const rows = (data as Pick<FitMeasurement, "metric" | "label" | "unit" | "category">[] | null) ?? [];
...
export async function getBaselineComparison(patientId: string): Promise<FitBaselineRow[]> {
  ...
  const { data: baseMeas } = await supabase
    .from("fit_measurements")
    .select("metric, label, unit, value")
    .eq("assessment_id", (baseline as { id: string }).id);
  const baseRows = (baseMeas as Pick<FitMeasurement, "metric" | "label" | "unit" | "value">[] | null) ?? [];
...
export async function getCurrentBMI(patientId: string): Promise<FitBMI | null> {
  const weightSeries = await getWeightSeries(patientId);
  if (weightSeries.length === 0) return null;
  const heightSeries = await getMeasurementSeries(patientId, "height_cm");
  if (heightSeries.length === 0) return null;
  const weight = weightSeries[weightSeries.length - 1].value;
  const height_cm = heightSeries[heightSeries.length - 1].value;
  if (!height_cm) return null;
  const m = height_cm / 100;
  return { bmi: weight / (m * m), weight, height_cm };
}
