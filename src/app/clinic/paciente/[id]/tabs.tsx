...
export function HistoricoSessoes({ criancaId }: { criancaId: string }) {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      setLoading(true)
      // Busca session_summary com join em sessoes_v2 para dados complementares
      ...
      // Busca dados complementares das sessoes_v2
      ...
      const result: SessaoHistorico[] = summaries.map(sm => {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {[
          ...
          { l: 'Taxa média',    v: `${Math.round(sessoes.filter(s => s.taxa_geral !== null).reduce((a,s) => a + (s.taxa_geral ?? 0), 0) / (sessoes.filter(s => s.taxa_geral !== null).length || 1))}%`, c: '#1D9E75' },
          ...
          <div key={s.id} style={{ ...card, overflow: 'hidden', border: `1px solid ${estaAberta ? cor + '44' : 'rgba(26,58,92,.5)'}` }}>
            ...
            <div onClick={() => setAberta(estaAberta ? null : s.id)}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                  ...
                  <span style={{ fontSize: '.68rem', color: 'rgba(160,200,235,.4)' }}>·</span>
                  <span style={{ fontSize: '.68rem', color: 'rgba(160,200,235,.4)' }}>{fmt(s.duracao_segundos)}</span>
                  {s.acrescimo_min > 0 && (
                    <span style={{ fontSize: '.62rem', color: '#EF9F27' }}>+{s.acrescimo_min}min</span>
                  )}
                </div>
                <div style={{ fontSize: '.68rem', color: 'rgba(160,200,235,.4)' }}>
                  {s.terapeuta_nome}
                  {s.total_operantes ? ` · ${s.total_operantes} operantes` : ''}
                </div>
              </div>
              ...
              <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="rgba(160,200,235,.3)" strokeWidth="1.5" style={{ transform: estaAberta ? 'rotate(180deg)' : 'none', transition: 'transform .2s', flexShrink: 0 }}>
                <path d="M3 6l5 5 5-5" />
              </svg>
            </div>
            ...
            {estaAberta && (
              <div style={{ borderTop: '1px solid rgba(26,58,92,.3)', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                      ...
                      {progInterv.map((p:any, i:number) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: 'rgba(26,58,92,.2)', borderRadius: 8 }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
                            <div style={{ fontSize: '1rem', fontWeight: 800, color: p.taxa >= 80 ? '#1D9E75' : p.taxa >= 50 ? '#EF9F27' : '#E05A4B' }}>
                              {p.taxa}%
                            </div>
                ...
                {progAval.length > 0 && (
                  <div>
                    <div style={{ fontSize: '.65rem', color: 'rgba(170,210,245,.5)', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 8 }}>Avaliações</div>
...
export function ContratoTab({ criancaId, terapeutaId }: { criancaId: string; terapeutaId: string }) {
  ...
  const [modalidades, setModalidades] = useState([
    { tipo: 'atendimento', valor_sessao: 250, sessoes_por_semana: 2, duracao_min: 60, total_sessoes: null as number|null, local_padrao: 'presencial', ativo: true },
  ])
  ...
  async function criarContrato() {
    ...
    try {
      ...
      for (const m of modalidades) {
        if (!m.ativo) continue
        await supabase.from('contrato_modalidades').insert({
          contrato_id:        c.id,
          tipo:               m.tipo,
      ...
      // Recarregar
      const { data: mods } = await supabase.from('contrato_modalidades').select('*').eq('contrato_id', c.id)
      setContrato({ id: c.id, status: 'ativo', data_inicio: form.data_inicio, data_fim: form.data_fim || null, modalidades: mods ?? [] })
  ...
  function addModalidade() {
    setModalidades(prev => [...prev, { tipo: 'acompanhamento_terapeutico', valor_sessao: 200, sessoes_por_semana: 1, duracao_min: 120, total_sessoes: null, local_padrao: 'ambiente_natural', ativo: true }])
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 20 }}>
            ...
            return (
              <div key={m.id} style={{ ...card, padding: 18, border: `1px solid ${cor}22` }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                  ...
                  <div style={{ fontSize: '.9rem', fontWeight: 800, color: '#e8f0f8' }}>{fmtBRL(m.valor_sessao)}<span style={{ fontSize: '.65rem', color: 'rgba(160,200,235,.4)', fontWeight: 400 }}>/sessão</span></div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: progresso !== null ? 12 : 0 }}>
                  ...
                  <div style={{ padding: '8px 10px', background: 'rgba(26,58,92,.2)', borderRadius: 7 }}>
                    ...
                    <div style={{ fontSize: '.85rem', fontWeight: 700, color: '#e8f0f8' }}>
                      {m.sessoes_realizadas}/{m.total_sessoes ?? '∞'}
                    </div>
                  </div>
                </div>
                ...
                {progresso !== null && (
                  <>
                    <div style={{ height: 4, background: 'rgba(26,58,92,.5)', borderRadius: 2, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${progresso}%`, background: cor, borderRadius: 2 }} />
                    </div>
                    <div style={{ fontSize: '.65rem', color: 'rgba(160,200,235,.4)', marginTop: 4 }}>
                      {progresso}% do contrato utilizado
                    </div>
                  </>
                )}
              </div>
      ...
      {criandoModal && (
        <div onClick={() => setCriandoModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.65)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 50, padding: 20 }}>
          <div onClick={e => e.stopPropagation()} style={{ ...card, padding: 28, width: '100%', maxWidth: 540, maxHeight: '90vh', overflowY: 'auto', border: '1px solid rgba(99,179,237,.15)' }}>
            ...
            <div style={{ fontSize: '.75rem', color: 'rgba(160,200,235,.4)', marginBottom: 24 }}>
              O contrato define modalidades, frequência e valores para este paciente.
            </div>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              {modalidades.map((m, idx) => {
                const cor = TIPO_COR[m.tipo] ?? '#1D9E75'
                return (
                  <div key={idx} style={{ padding: 16, borderRadius: 10, border: `1px solid ${cor}22`, background: cor + '08', marginBottom: 10 }}>
                    {/* Tipo */}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      <div>
                        ...
                        <input type="number" value={m.valor_sessao} onChange={e => setModalidades(prev => prev.map((x,i) => i===idx ? { ...x, valor_sessao: parseFloat(e.target.value) } : x))} style={inp} />
                      </div>
                      <div>
                        <label style={{ fontSize: '.6rem', color: 'rgba(170,210,245,.4)', display: 'block', marginBottom: 4 }}>Sessões/semana</label>
                        <input type="number" min={0.5} max={7} step={0.5} value={m.sessoes_por_semana} onChange={e => setModalidades(prev => prev.map((x,i) => i===idx ? { ...x, sessoes_por_semana: parseFloat(e.target.value) } : x))} style={inp} />
                      </div>
                      <div>
                        <label style={{ fontSize: '.6rem', color: 'rgba(170,210,245,.4)', display: 'block', marginBottom: 4 }}>Duração (min)</label>
                        <input type="number" min={30} max={240} step={15} value={m.duracao_min} onChange={e => setModalidades(prev => prev.map((x,i) => i===idx ? { ...x, duracao_min: parseInt(e.target.value) } : x))} style={inp} />
                      </div>
                      <div>
                        <label style={{ fontSize: '.6rem', color: 'rgba(170,210,245,.4)', display: 'block', marginBottom: 4 }}>Total sessões (opcional)</label>
                        <input type="number" min={1} placeholder="∞ aberto" value={m.total_sessoes ?? ''} onChange={e => setModalidades(prev => prev.map((x,i) => i===idx ? { ...x, total_sessoes: e.target.value ? parseInt(e.target.value) : null } : x))} style={inp} />
                      </div>
                    </div>
                    ...
                    {idx > 0 && (
                      <button onClick={() => setModalidades(prev => prev.filter((_,i) => i !== idx))} style={{ marginTop: 8, fontSize: '.65rem', color: '#E05A4B', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>
                        Remover modalidade
                      </button>
                    )}
                  </div>
              ...
              <button onClick={addModalidade} style={{ width: '100%', padding: '9px', borderRadius: 9, border: '1px dashed rgba(26,58,92,.5)', background: 'transparent', color: 'rgba(160,200,235,.4)', fontSize: '.75rem', cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setCriandoModal(false)} style={{ flex: 1, padding: 11, borderRadius: 9, border: '1px solid rgba(26,58,92,.5)', background: 'transparent', color: 'rgba(160,200,235,.5)', fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font-sans)' }}>
                Cancelar
              </button>
              <button onClick={criarContrato} disabled={salvando} style={{ flex: 2, padding: 11, borderRadius: 9, border: 'none', background: salvando ? 'rgba(29,158,117,.4)' : 'linear-gradient(135deg,#1D9E75,#0f8f7a)', color: '#07111f', fontSize: 13, fontWeight: 700, cursor: salvando ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-sans)' }}>
